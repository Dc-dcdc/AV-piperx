...
"""An online buffer for online training loops.
...
class OnlineBuffer(torch.utils.data.Dataset):
    """FIFO data buffer for an online training loop.
    ...
    def add_data(self, data: dict[str, np.ndarray]):
        ...
        start from 0 (note to the developer: this can easily be generalized). Rollout producers should
        construct data using the same field layout as a `LeRobotDataset` episode.
...
def compute_sampler_weights(
    ...
    """Compute sampling weights for an online training dataloader.
