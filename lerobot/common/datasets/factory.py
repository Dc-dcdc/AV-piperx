...
'''
读取用户的配置文件（Hydra Config），并根据这些配置自动化地构建、
装配和返回一个可以直接用于神经网络训练的数据集对象（LeRobotDataset 或 MultiLeRobotDataset）
'''
import logging
...
def make_dataset(cfg, split: str = "train") -> LeRobotDataset | MultiLeRobotDataset:
    ...
    if isinstance(cfg.dataset_repo_id, str):
        dataset = LeRobotDataset(
            cfg.dataset_repo_id,    #根据id下载或者加载本地数据（/home/dc/.cache/huggingface/datasets）
            split=split,
            delta_timestamps=cfg.training.get("delta_timestamps"),
