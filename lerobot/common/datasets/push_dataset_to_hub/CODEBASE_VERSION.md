...
compatible with previous codebase versions. The dataset upload pipeline should automatically tag the dataset
with the current `CODEBASE_VERSION`.

### Updating an existing dataset
If you want to update an existing dataset, you need to change the `CODEBASE_VERSION` from `lerobot_dataset.py`
before running your conversion and upload pipeline. This is especially useful if you introduce a breaking change
...
that doesn't require re-encoding or re-uploading the dataset. You can just "branch-out" from the `main` branch on HF
