...
该文件保留原始 LeRobot Diffusion Policy 的观测编码和队列逻辑，
但将 action 拆成双臂动作头和头部/视角动作头分别建模。
外部数据字段仍然使用完整的 `action`，因此数据格式不需要改变。
"""
...
_DEBUG_INPUT_IMAGES_DIR = Path(__file__).resolve().parents[4] / "input_images"


class DualHeadDiffusionPolicy(
    nn.Module,
    ...
    拆分维度可通过 config.arm_action_dim、config.view_action_dim、config.view_loss_weight 覆盖。
    """
    ...
    def _make_diffusion_model(self, config: DiffusionConfig) -> "DualHeadDiffusionModel":
        """Construct the diffusion core.

        Kept as a factory hook so action-space variants can reuse the policy's
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        underlying diffusion model. Here's how it works:
          ...
          - `n_action_steps` worth of actions are kept from the last cached observation, following the
            original Diffusion Policy convention.
        ...
        action = self._queues["action"].popleft()  # 从动作队列中取出第一个动作，这里一次生成8步的动作，一个个取
        return action
...
class DualHeadDiffusionModel(nn.Module):
    def __init__(self, config: DiffusionConfig):
        ...
        if self.arm_action_dim + self.view_action_dim != self.action_dim:
            raise ValueError(
                f"Dual-head action dims do not match output action dim: "
        ...
        self.arm_unet = DiffusionConditionalUnet1d(
            self._make_head_config(self.arm_action_dim), global_cond_dim=global_cond_dim
        )
        self.view_unet = DiffusionConditionalUnet1d(
            self._make_head_config(self.view_action_dim), global_cond_dim=global_cond_dim
        )

        self.arm_noise_scheduler = self._make_noise_scheduler()
        self.view_noise_scheduler = self._make_noise_scheduler()

        if config.num_inference_steps is None:
            self.num_inference_steps = self.arm_noise_scheduler.config.num_train_timesteps
        else:
            self.num_inference_steps = config.num_inference_steps

    def _make_head_config(self, action_dim: int) -> DiffusionConfig:
        """Create a shallow config copy with the requested per-head action dimension."""
    ...
    def _make_noise_scheduler(self) -> DDPMScheduler | DDIMScheduler:
        """Create an independent noise scheduler for one action head."""
    ...
    def _prepare_head_trajectories(self, actions: Tensor) -> tuple[Tensor, Tensor]:
        """Map a normalized full action trajectory to the two diffusion targets."""
        return (
    ...
    def conditional_sample(
        ...
        # Sample prior for this action head.
        sample = torch.randn(
    ...
    def generate_actions(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # Encode image features once and share them between the two action heads.
        ...
        if start < 0 or end > actions.shape[1]:
            raise ValueError(
                f"Invalid action slice [{start}:{end}] for horizon={actions.shape[1]}"
            )
    ...
    def compute_loss(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        ...
        This function expects `batch` to have (at least):
        {
            ...
            "loss_mask": optional (B, horizon, action_dim), per-element IWR weight
        }
        """
        # Input validation.
        ...
        if loss_mask is not None and loss_mask.shape != batch["action"].shape:
            raise ValueError(
                "loss_mask must have the same [B, horizon, action_dim] shape as action, "
                f"got mask={tuple(loss_mask.shape)} action={tuple(batch['action'].shape)}."
            )

        # Encode image features once and share them between the two action heads.
        global_cond = self._prepare_global_conditioning(batch)  # (B, global_cond_dim)

        arm_trajectory, view_trajectory = self._prepare_head_trajectories(batch["action"])
        arm_loss_mask = None
        view_loss_mask = None
    ...
    def _compute_head_loss(
        ...
        *,
        ...
        pred = unet(noisy_trajectory, timesteps, global_cond=global_cond)
        ...
        weights = None
        if loss_mask is not None:
            weights = loss_mask.to(device=loss.device, dtype=loss.dtype)
            if torch.any(weights < 0):
                raise ValueError("loss_mask weights must be non-negative.")
        ...
        if self.config.do_mask_loss_for_padding:
            ...
            padding_weights = in_episode_bound.unsqueeze(-1).to(dtype=loss.dtype)
...
class DiffusionRgbEncoder(nn.Module):
    ...
    def __init__(self, config: DiffusionConfig):
        ...
        # ==========================================
        # 添加的resize_shape修改
        if getattr(config, "resize_shape", None) is not None:
            self.do_resize = True
            # 加入 antialias=True 保证缩小图像时抗锯齿，画面更清晰
        ...
        if config.crop_shape is not None:
            ...
            self.do_crop = False
        self.debug_save_input_images = bool(getattr(config, "debug_save_input_images", False))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # 设置只保存前 3 个批次的图片，防止跑满硬盘
        is_saving_step = self.debug_save_input_images and self._debug_img_counter < 3

        if is_saving_step:
            _DEBUG_INPUT_IMAGES_DIR.mkdir(parents=True, exist_ok=True)
            path_resize = _DEBUG_INPUT_IMAGES_DIR / f"debug_{self._debug_img_counter:04d}_step1_resized.png"
            torchvision.utils.save_image(x[0], path_resize)
        # ==========================================
        ...
        # ------------------------------------------
        # 📸 [调试保存 2]: 记录 Crop 之后的状态
        # ------------------------------------------
        if is_saving_step:
            path_crop = _DEBUG_INPUT_IMAGES_DIR / f"debug_{self._debug_img_counter:04d}_step2_cropped.png"
            torchvision.utils.save_image(x[0], path_crop)
            print(f"📸 [Debug] 已保存对比组 {self._debug_img_counter:04d}: 包含 Resized 和 Cropped 图像")
