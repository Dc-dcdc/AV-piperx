...
from train.s3_finetune.finetune_mlp import FrozenDiffusionMLPPolicy
...
def load_frozen_diffusion_mlp_policy(base_policy, load_dir: str | Path, device):
    ...
    residual_mlp_mode = str(pick("residual_mlp_mode", "split")).lower()
    residual_mlp_condition = str(pick("residual_mlp_condition", "action")).lower()
    residual_mlp_stepwise_obs = bool(pick("residual_mlp_stepwise_obs", False))
    ...
    if residual_mlp_mode in {"all", "single"}:
        residual_mlp_mode = "joint"
    if residual_mlp_mode == "joint":
        missing_keys = ["mlp_all"] if "mlp_all" not in mlp_ckpt else []
    else:
        missing_keys = [key for key in ("mlp_arm", "mlp_cam") if key not in mlp_ckpt]
    if missing_keys:
        raise ValueError(
            f"{mlp_ckpt_path} 不是当前 residual MLP 权重格式，缺少 {missing_keys}。"
        )
    ...
    if policy.residual_mlp_mode == "joint":
        policy.mlp_all.load_state_dict(mlp_ckpt["mlp_all"], strict=True)
    elif policy.arm_action_indices:
        policy.mlp_arm.load_state_dict(mlp_ckpt["mlp_arm"], strict=True)
    if policy.residual_mlp_mode == "split" and policy.cam_action_indices:
    ...
    logging.info(
        ...
        f"mode={policy.residual_mlp_mode}, "
        f"condition={policy.residual_mlp_condition}, stepwise_obs={policy.residual_mlp_stepwise_obs}, "
        f"global_cond_dim={policy.residual_global_cond_dim}, "
        f"arm_indices={policy.arm_action_indices}, cam_indices={policy.cam_action_indices}, "
        f"lambda_all={policy.lambda_all:.3f}, "
        f"lambda_arm={policy.lambda_arm:.3f}, lambda_cam={policy.lambda_cam:.3f}, "
        f"max_delta={policy.residual_mlp_max_delta:.5f}, "
...
if __name__ == "__main__":
    ...
    eval_cfg = SimpleNamespace(
        ...
        ckpt_path="outputs/3_finetune/train/2026-07-03/13-22-00_InsertCylinder-3Arms-v0_ft_zed_diffusion_mlp/checkpoints/000020_sr=0.70_reward=695.89_MLPloss=0.0133_Vloss=1.6354",
        # ⚙️ 评估参数设置
