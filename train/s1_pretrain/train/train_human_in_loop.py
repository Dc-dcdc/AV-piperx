...
from train.s1_pretrain.eval.eval_train import evaluate_and_checkpoint_if_needed, TopKCheckpointManager, make_eval_env
...
def make_optimizer_and_scheduler(cfg, policy):
    if cfg.policy.name == "act":
        ...
        backbone_named_parameters = [
            ...
            if is_visual_backbone_parameter(name)
        ]
        ...
        logging.info(
            ...
            f"参数组: main={len(main_parameters)} tensors "
...
def train_human_in_loop(cfg: DictConfig, out_dir: str | None = None, job_name: str | None = None):
    ...
    if cfg.resume:
        ...
        if configured_use_ema:
            online_policy_path = checkpoint_dir / "online_pretrained_model"
            ema_policy_load_path = checkpoint_dir / "pretrained_model"
            if (
                not online_policy_path.is_dir()
                or not ema_policy_load_path.is_dir()
            ):
                raise FileNotFoundError(
                    "严格EMA续训需要checkpoint同时包含"
                    "online_pretrained_model和pretrained_model。"
                    "旧checkpoint没有真实EMA历史，请将其作为首次HIL的"
                    "pretrained_model_path，而不要设置resume=true。"
                )
            policy_load_path = online_policy_path
    ...
    if cfg.resume:
        ...
        if ema is not None:
            ...
            ema.restore(ema_loaded_policy, checkpoint_dict.get("ema"))
